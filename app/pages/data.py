...
def update_output(contents, list_of_names, list_of_dates):
    ...
    logging.info(
        f"File uploaded with name: '{list_of_names}' and date: {list_of_dates}")
    ...
    try:
        if list_of_names.endswith('.csv'):
            ...
            f.close()
            EventData.uploaded_log = pd.read_csv(
                io.StringIO(decoded.decode('utf-8')))
            feather.write_feather(EventData.uploaded_log,
                                  'app/assets/temp/uploaded.feather')
