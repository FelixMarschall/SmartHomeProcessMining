...
layout = html.Div(
    [
            ...
            children=[
                html.Div(
                    children=[
                        ...
                        html.P(
                            'Activity column (e.g. "concept:name" or "name_state_id")'
                        ),
                        dcc.Dropdown(
                        ...
                        html.P(
                            'Case ID column (e.g. "case:concept:name" or "single_day_id_H")'
                        ),
                        dcc.Dropdown(
                            id="case-id-dropdown",
                            clearable=False,
        ...
        dcc.Loading(
            id="loading-1", type="circle", children=html.Div(id="loading-output-1")
        ),
        html.H2("Parameters"),
        html.Div(
            id="alpha-parameters",
            children="No parameters available for Alpha Miner",
            hidden=True,
        ),
        html.Div(
            id="inductive-parameters",
            children=[
                html.Label(
                    "Noise Threshold",
                    style=transformation_components.get_parameter_input_style(),
                ),
                dcc.Input(
                    id="noise_threshold",
                    type="number",
        ...
        html.Hr(),
        html.Button(
            "Start mining", id="mine-button", style=components.get_button_style()
        ),
        ...
        html.Div(
            ...
            children=[
                ...
                dbc.Collapse(
                    [
                        html.P(
                            "Graph is not influenced by the transformation algorithm or the parameters above."
                        ),
...
def update_datasource_dropdown(data_source):
    ...
    options = [
        {
            "label": "Uploaded Event Log",
            "value": "uploaded_log",
            "disabled": not uploaded_log_is_available,
        },
        {
            "label": "Home Assistant",
            "value": "home-assistant",
            "disabled": not logbook_is_available,
        },
...
def update_transformation(
    ...
    logging.debug(
        f"Callback 'start mining' button with value: {value} and algo: {algo}"
    )
    ...
    try:
        ...
        if algo == "alpha":
            ...
            if noise_threshold is None or noise_threshold > 1 or noise_threshold < 0:
                noise_threshold = 0.0
                logging.debug(
                    "Noise threshold is not set or not in range [0,1], using default value 0.0"
                )
            ...
            if (
                dependency_threshold is None
                or dependency_threshold > 1
                or dependency_threshold < 0
            ):
                dependency_threshold = 0.5
            if and_threshold is None or and_threshold > 1 or and_threshold < 0:
                and_threshold = 0.65
            if (
                loop_two_threshold is None
                or loop_two_threshold > 1
                or loop_two_threshold < 0
            ):
    ...
    try:
        ...
        pt = None
        logging.error(
            type(e).__name__ + " converting process model to process tree: " + str(e)
        )
...
def update_graphs(ts, image_file_name):
    ...
    return (
        dash.get_asset_url(image_file_name["dfg"]),
        dash.get_asset_url(image_file_name["petri"]),
        dash.get_asset_url(image_file_name["bpmn"]),
        dash.get_asset_url(image_file_name["tree"]),
    )
